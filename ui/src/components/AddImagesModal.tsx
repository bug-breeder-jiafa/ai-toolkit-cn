              ...
              <div className="text-center">
                ...
                <div {...getRootProps()} className="w-full">
                  ...
                  >
                    ...
                    {!isUploading ? (
                      <>
                        <p className="text-sm text-gray-200 text-center">将文件拖放到此处或点击选择</p>
                        <p className="text-xs text-gray-400 mt-1">支持图片、视频或 .txt 文件</p>
                      </>
                    ) : (
                      <p className="text-sm text-gray-200 text-center">继续拖放文件添加到队列</p>
                    )}
                  </div>
                </div>
  ...
  return (
    <div className="flex items-center gap-2 px-3 text-xs font-mono" style={{ height: ROW_HEIGHT }}>
      ...
      <span className="flex-shrink-0 w-16 text-right">
        ...
        {entry.status === 'pending' && <span className="text-white/30">等待中</span>}
      </span>
    </div>
  );
}
